...
gulp.task('packageTask', ['validate'], function() {
    ...
    function work(clicktag, size, v, language, revision) {
        ...
        var packageName = `${year}_${brand}Brand_RL_Other_${name}_Brand${month}_HTML5_CA_${language.toUpperCase()}_V${v}${!!revision?revision:''}_${size}`
    ...
    for (var h in config.sizes) {
        for (var j in config.clicktags) {
            for (var i in config.text) {
                ...
                var language = i
                var v = version[language]
                var imageName = `${year}_${brand}Brand_RL_Other_${name}_Brand${month}_HTML5_CA_${language.toUpperCase()}`
...
gulp.task('packageContinueTask', ['packageStaticTask'], function() {
    ...
    for (var j in config.clicktags) {
        for (var i in config.text) {
            ...
            var clicktag = config.clicktags[j]
            var v = version[i]
            var n = `${year}_${brand}Brand_RL_Other_${name}_Brand${month}_HTML5_CA_${clicktag}_${language.toUpperCase()}`
